...
@:allow(Quickview)
@:allow(Session)
class AssetPacker extends luxe.Game {
    ...
    override function config(config:luxe.GameConfig) {
        ...
        config.preload.textures = [
            { id:'assets/selected.png', filter_min:nearest, filter_mag:nearest },
            { id:'assets/selecteds.png', filter_min:nearest, filter_mag:nearest },
            { id:'assets/iconmonstr-add-folder-2-icon-32.png' },
            { id:'assets/iconmonstr-folder-2-icon-32.png' },
            { id:'assets/iconmonstr-refresh-2-icon-32.png' },
            { id:'assets/iconmonstr-refresh-3-icon-32.png' },
            { id:'assets/iconmonstr-save-3-icon-32.png' },
            { id:'assets/iconmonstr-selection-10-icon-32.png' },
            { id:'assets/iconmonstr-selection-11-icon-32.png' },
            { id:'assets/iconmonstr-shipping-box-10-icon-32.png' },
            { id:'assets/iconmonstr-shipping-box-9-icon-32.png' },
            { id:'assets/iconmonstr-sound-wave-icon-128.png' },
        ];

        var _r = def(config.user, {});
    ...
    function create_top_menu() {
        ...
        var _selectall = new mint.Image({
            parent: canvas,
            name: 'select_all',
            x:16+(48*6),y:20,w:24,h:24,mouse_input: true,
        ...
        var _build = new mint.Image({
            parent: canvas,
            name: 'build',
            x:16+(48*9),y:20,w:24,h:24,mouse_input: true,
        ...
        menu_label = new mint.Label({
            ...
            x:16+(48*10), y:20, w:Luxe.screen.w-(16+(48*10))-220, h:24,
            text_size: 24,
        });
    ...
    function create_right_menu() {
        ...
        new mint.Label({
            ...
            text: 'simple asset packer 0.3.0',
    ...
    function build(_,_) {
        ...
        for(node in selected) {
            ...
            p.then(function(b:snow.systems.assets.Asset.AssetBytes) {
        ...
        Promise.all(wait).then(function(_){
            ...
            var save_path = Dialogs.save('select parcel file to save to', { ext:'parcel' });
...
*/
    ...
    function click_open_folder(_,_) {
        ...
        var _path = Dialogs.folder('select assets folder to show');
        if(_path.length > 0) {
                ...
                //controls
                    ...
                    if(is_texture(ext)) {
                        image = new mint.Image({
                            ...
                            visible: true,
                            options: {
                                sizing: mint.render.luxe.Image.ImageSizing.cover
                            },
    ...
    function load_files() {
        ...
        for(_type in assettypes) {
            ...
            if(items.length > 0) {
                for(item in items) {
                    var ext = haxe.io.Path.extension(item.full_path);
                    if(is_texture(ext)) {
                        Luxe.resources.load_texture(item.full_path);
                    }
                }
            }
        }
